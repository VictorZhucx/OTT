...
struct snd_soc_aux_dev m8_audio_aux_dev;
static struct snd_soc_codec_conf m8_audio_codec_conf[] = {
	{
		.name_prefix = "AMP",
	},
};
static struct codec_info m8_codec_info_aux;
...
static int aml_aux_dev_parse_of(struct snd_soc_card *card)
{
	...
	if (get_audio_codec_i2c_info(child, &temp_audio_codec) == 0) {
		...
		strlcpy(m8_codec_info_aux.name, tmp, I2C_NAME_SIZE);
		snprintf(tmp, I2C_NAME_SIZE, "%s.%s", temp_audio_codec.name,
				dev_name(&client->dev));
		strlcpy(m8_codec_info_aux.name_bus, tmp, I2C_NAME_SIZE);

		m8_audio_aux_dev.name = m8_codec_info_aux.name;
		m8_audio_aux_dev.codec_name = m8_codec_info_aux.name_bus;
		m8_audio_codec_conf[0].dev_name = m8_codec_info_aux.name_bus;

		card->aux_dev = &m8_audio_aux_dev,
		card->num_aux_devs = 1,
		card->codec_conf = m8_audio_codec_conf,
		card->num_configs = ARRAY_SIZE(m8_audio_codec_conf);
        pr_info("====board_info.type = %s\n", board_info.type);

...
#endif
struct i2c_client *g_aml_es8374_client = NULL;
struct sp_config {
	u8 spc, mmcc, spfs;
	u32 srate;
	u8 lrcdiv;
...
int es8374_i2c_read(uint8_t reg)
{
	int ret = 0;
	uint8_t acBuf[1] = {0};
	uint8_t acData[1] = {0};
	...
	acBuf[0] = reg;
	ret = i2c_transfer(g_aml_es8374_client->adapter, msgs, 2);
    if (ret < 0)
    {
        pr_err("%s: i2c_transfer(read) err, reg is 0x%x, val is 0x%x, ret=%d", __func__, reg, acData[0], ret);
		return ret;
    }
    pr_info("%s: i2c_transfer(read), reg is 0x%x, val is 0x%x, ret=%d", __func__, reg, acData[0], ret);
	return 0;
}

int es8374_i2c_write(uint8_t reg, uint8_t val)
{
    int ret = 0;
	...
	acBuf[0] = reg;
	acBuf[1] = val;
    ret = i2c_transfer(g_aml_es8374_client->adapter, msgs, 1);
    if (ret < 0)
    {
...
static const DECLARE_TLV_DB_SCALE(adc_rec_tlv, 0, 50, 0);
/*
...
SOC_VALUE_ENUM_SINGLE(ES8374_AIN_PWR_SRC_REG21, 4, 1,
			...
			int source, unsigned int freq_in, unsigned int freq_out)
{
	...
	if (source == ES8374_PLL_SRC_FRM_MCLK) {
		...
		reg &= 0xF0;
		reg |= (N & 0x0F);
		es8374_i2c_write(ES8374_PLL_N_REG0B, reg);

		K1 &= 0x3F;
		es8374_i2c_write(ES8374_PLL_K_REG0C, K1);
		es8374_i2c_write(ES8374_PLL_K_REG0D, K2);
		es8374_i2c_write(ES8374_PLL_K_REG0E, K3);
		...
		reg &= 0xfc;
		reg |= 0x01;
		es8374_i2c_write(ES8374_PLL_CONTROL1_REG09, reg);
			...
			int clk_id, unsigned int freq, int dir)
{
	...
	if (clk_id == ES8374_CLKID_MCLK) {
		es8374_i2c_write(0x01,0x7F);	//IC clk on
		es8374_i2c_write(0x05,0x11);	//clk div set
		es8374_i2c_write(0x09,0x01);	//pll set:reset on ,set start
		es8374_i2c_write(0x0C,0x22);	//pll set:k
		es8374_i2c_write(0x0D,0x2E);	//pll set:k
		es8374_i2c_write(0x0E,0xC6);	//pll set:k
...
static int es8374_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	es8374_i2c_write(ES8374_MS_BCKDIV_REG0F, iface);
	es8374_i2c_write(ES8374_ADC_FMT_REG10, adciface);
	es8374_i2c_write(ES8374_DAC_FMT_REG11, daciface);
	return 0;
}
			...
			struct snd_soc_dai *dai)
{
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		/* set iface & srate */
		es8374_i2c_write(ES8374_DAC_FMT_REG11, iface);
	} else {
		...
		/* set iface */
		es8374_i2c_write(ES8374_ADC_FMT_REG10, iface);
	}

	if(es8374->dmic_enable){
		printk("dmic is enabled\n");
		es8374_i2c_write(ES8374_GPIO_INSERT_REG6D, 0x5F);  //set gpio1 to DMIC CLK
	} else {
		printk("dmic is not enabled\n");
		es8374_i2c_write(ES8374_GPIO_INSERT_REG6D, 0x1F);  //set gpio1 to GM SHORT
	}
...
//	struct es8374_private *es8374 = snd_soc_codec_get_drvdata(codec);
	...
	pr_info("enter es8374_set_bias_level\n");

	switch (level) {
	case SND_SOC_BIAS_ON:
		pr_info("enter SND_SOC_BIAS_ON\n");
		es8374_i2c_write(ES8374_CLK_MANAGEMENT_REG01, 0x7f);
		es8374_i2c_write(ES8374_ANA_REF_REG14, 0x8a);
		es8374_i2c_write(ES8374_ANA_PWR_CTL_REG15, 0x40);
		snd_soc_update_bits(codec, ES8374_AIN_PWR_SRC_REG21, 0xc0, 0x00);
		es8374_i2c_write(ES8374_MONO_MIX_REG1A, 0xa0);
		es8374_i2c_write(ES8374_SPK_MIX_REG1C, 0x90);
		es8374_i2c_write(ES8374_SPK_MIX_GAIN_REG1D, 0x02);
		es8374_i2c_write(ES8374_SPK_OUT_GAIN_REG1E, 0xa0);
		es8374_i2c_write(ES8374_DAC_CONTROL_REG36, 0x00);
		es8374_i2c_write(ES8374_DAC_CONTROL_REG37, 0x00);
		es8374_i2c_write(ES8374_DAC_VOLUME_REG38, 0x00);
		es8374_i2c_write(ES8374_ADC_VOLUME_REG25, 0x00);
		break;
	...
	case SND_SOC_BIAS_STANDBY:
		pr_info("enter== SND_SOC_BIAS_STANDBY\n");
		/*
		es8374_i2c_write(ES8374_DAC_VOLUME_REG38, 0xc0);
		es8374_i2c_write(ES8374_ADC_VOLUME_REG25, 0xc0);
		es8374_i2c_write(ES8374_DAC_CONTROL_REG36, 0x20);
		es8374_i2c_write(ES8374_DAC_CONTROL_REG37, 0x21);
		es8374_i2c_write(ES8374_MONO_MIX_REG1A, 0x08);
		es8374_i2c_write(ES8374_SPK_MIX_REG1C, 0x10);
		es8374_i2c_write(ES8374_SPK_MIX_GAIN_REG1D, 0x10);
		es8374_i2c_write(ES8374_SPK_OUT_GAIN_REG1E, 0x40);
	*/	
		break;

	case SND_SOC_BIAS_OFF:
		pr_info("enter SND_SOC_BIAS_OFF\n");
	/*	
		es8374_i2c_write(ES8374_ALC_LVL_HLD_REG28, 0x1c);
		snd_soc_update_bits(codec, ES8374_AIN_PWR_SRC_REG21, 0xc0, 0x00);
		es8374_i2c_write(ES8374_ANA_PWR_CTL_REG15, 0xbf);
		es8374_i2c_write(ES8374_ANA_REF_REG14, 0x14);
		es8374_i2c_write(ES8374_CLK_MANAGEMENT_REG01, 0x03);
	*/	
		break;
	}
...
static int es8374_probe(struct snd_soc_codec *codec)
{
	...
	//if (ret < 0)
	//	return ret;
	pr_info("es8374_probe==\n");

	es8374->codec = codec;
	snd_soc_codec_set_drvdata(codec, es8374);

	ret = es8374_i2c_write(0x00,0x3F);	//IC Rst start
	msleep(1);  					//DELAY_MS
	ret = es8374_i2c_write(0x00,0x03);	//IC Rst stop
	ret = es8374_i2c_write(0x01,0x7F);	//IC clk on
	ret = es8374_i2c_write(0x05,0x11);	//clk div set
	ret = es8374_i2c_write(0x6F,0xA0);	//pll set:mode enable
	ret = es8374_i2c_write(0x72,0x41);	//pll set:mode set
	ret = es8374_i2c_write(0x09,0x01);	//pll set:reset on ,set start
	ret = es8374_i2c_write(0x0C,0x22);	//pll set:k
	ret = es8374_i2c_write(0x0D,0x2E);	//pll set:k
	ret = es8374_i2c_write(0x0E,0xC6);	//pll set:k
	ret = es8374_i2c_write(0x0A,0x3A);	//pll set:
	ret = es8374_i2c_write(0x0B,0x07);	//pll set:n
	ret = es8374_i2c_write(0x09,0x41);	//pll set:reset off ,set stop
	ret = es8374_i2c_write(0x24,0x08);	//adc set
	ret = es8374_i2c_write(0x36,0x00);	//dac set
	...
	ret = es8374_i2c_write(0x38,0x00);	// DACVOLUMEL on
	ret = es8374_i2c_write(0x37,0x00);	// dac set
	ret = es8374_i2c_write(0x6D,0x60);	//SEL:GPIO1=DMIC CLK OUT+SEL:GPIO2=PLL CLK OUT

	pr_info("probe printf ======");
	for (i = 0; i <= 0x4f; i++) {
		es8374_i2c_read(i);
	}
	return ret;
...
#ifdef es8374_dmic
	es8374->dmic_enable = 1;     /* if internal mic is dmic */
#else
	es8374->dmic_enable = 0;     /* if internal mic is amic */
#endif
		...
		const struct i2c_device_id *id)
{
	...
	g_aml_es8374_client = i2c_client;

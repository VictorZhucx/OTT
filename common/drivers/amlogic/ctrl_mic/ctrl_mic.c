...
static int aml_ctrl_spk_i2c_init(struct platform_device *pdev)
{
  ...
  if (val == 1) {
    ...
    //ctrl_spk_i2c_write(0x21, 0x24);
  } else {
    //ctrl_spk_i2c_write(0x21, 0x14);
...
static ssize_t ctrl_mic_send_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strncmp(buf, "1", 1) == 0) {
       ...
       //ctrl_mic_i2c_write(0x01, 0x7f);
	   //ctrl_mic_i2c_write(0x14, 0x8a);
	   //ctrl_mic_i2c_write(0x15, 0x40);
	   //snd_soc_update_bits(codec, ES8374_AIN_PWR_SRC_REG21, 0xc0, 0xc0);
       //ctrl_mic_i2c_write(0x1A, 0xa0);
	   //ctrl_mic_i2c_write(0x1c, 0x90);
	   //ctrl_mic_i2c_write(0x1d, 0x02);
	   //ctrl_mic_i2c_write(0x1e, 0xa0);
	   //ctrl_mic_i2c_write(0x36, 0x00); 
       //ctrl_mic_i2c_write(0x37, 0x00);
       //ctrl_mic_i2c_write(0x38, 0x00);
       //ctrl_mic_i2c_write(0x25, 0x00);
       for (i = 0; i <= 0x4f; i++) { 
	       ctrl_mic_i2c_read(i);
	   }
 	...
 	if (ctrl_mic_pin != 0) {
		...
		val = gpio_get_value(ctrl_mic_pin);
		if (val == 1) {
			pr_info("store ctrl mic is high\n");
		} else {
			pr_info("store ctrl mic is low\n");
		}
	}
	if (ctrl_spk_pin != 0) {
		val = gpio_get_value(ctrl_spk_pin);
		if (val == 1) {
			pr_info("store ctrl spk is high\n");
		} else {
			pr_info("store ctrl spk is low\n");
		}
	}
	return count;
}
